...
// Scores spot of board in terms of best placement (most tiles turned) for a given colour and spot
int scoreSpot (char board[][26], int n, char colour, char testRow, char testCol){
	// Declaring/initializing variables;
...
void computerMove(char board[][26], int n, char comColour){
	...
	for (testRow = 0; testRow < n; testRow ++){
		for (testCol = 0; testCol < n; testCol ++){
			...
			if (moveValid(board, n, comColour, testRow + 'a', testCol + 'a')){
				tempScore = scoreSpot(board, n, comColour, testRow + 'a', testCol + 'a');
