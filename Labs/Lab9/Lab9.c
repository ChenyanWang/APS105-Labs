...
void insertSong(Song library[], int size);
...
int main(void) {
    ...
    do {
        ...
        if (response == 'I') {
			...
			insertSong (library, librarySize);
			librarySize++;
    ...
    while(bottom < top && !sorted) {
        ...
        if (goingUp) {
            ...
            for(i = bottom; i < top; i++) {
				...
				// If artists are the same, check song name
...
// Inserts a song to end of array
void insertSong(Song library[], int size){
	...
	strcpy(library[size].songName, input);
	
	inputStringFromUser(promptArtist, input, MAX_LENGTH);
	strcpy(library[size].artist, input);
	
	inputStringFromUser(promptGenre, input, MAX_LENGTH);
	strcpy(library[size].genre, input);
